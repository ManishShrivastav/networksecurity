...
class DataValidation:
        ...
        try:
            ...
            for column in base_df.columns:
                ...
                if threshold < is_same_dist.pvalue:
                    ...
                    is_found = True
                    status = False
                report.update({column: {"drift_status": is_found, "pvalue": is_same_dist.pvalue}})
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            # read the data from training and testing file path
            train_dataframe = DataValidation.read_data(train_file_path)
            test_dataframe = DataValidation.read_data(test_file_path)

            # Validate the number of columns in the dataframes
            status = self.validate_number_of_columns(dataframe=train_dataframe)
            if not status:
                error_message = f"Train datafram does not contain all the required columns.\n"
            status = self.validate_number_of_columns(dataframe=test_dataframe)
            if not status:
                error_message = f"Test datafram does not contain all the required columns.\n"
